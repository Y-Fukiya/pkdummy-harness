...
- `python tools/validate_simulation.py outputs/<run>/raw/sim_full.csv --pk drugs/<slug>/pk.yml --targets drugs/<slug>/targets.yml --max-loops 3`: 生成済み濃度データから AUC/Cmax/Tmax/t1/2 を再計算して比較し、警告/失敗時は最大3回まで再点検履歴を残す。入力CSVに単位列があればレポートとCL由来AUC換算に使う。再点検は同じ計算の反復であり、最適化やcalibrationは行わない
