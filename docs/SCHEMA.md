...
`source_verification` を付ける場合は、`status`、`blocker`、`next_action`、`reviewed_source_ids`、`fixture_value_decision`、`decision_reason` も validator で確認されます。`status: no_exact_public_source_match` の entry は、source を推測接続しないため `source_id: null` のままにする必要があります。
...
value_provenance_summary:
  ...
  source_checked_fields:
    - t_half_h
  checked_fields:  # legacy alias for metadata_present_fields; validator requires it to match
    - CL_abs_L_per_h_at_70kg
    - V_abs_L_at_70kg
    - t_half_h
