...
| `value_provenance_summary.metadata_present_fields` | provenance metadata と canonical value との整合を確認できた field |
| `value_provenance_summary.source_checked_fields` | 非null `source_id` があり source review 済みの field |
| `value_provenance_summary.checked_fields` | 後方互換の legacy alias。新規監査では `metadata_present_fields` を優先 |
