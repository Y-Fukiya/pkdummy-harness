...
`--report` は `fields_needing_review` に加えて、`source_review_status` / `fixture_limitation_status` の集計、非null `source_id` entry数、`resolved_entries`、`resolved_source_refs`、`unresolved_entries`、`unresolved_entry_details`、`unresolved_reason_counts`、`source_mapping_coverage`、`source_mapping_coverage_by_field`、`source_mapping_coverage_by_drug`、`next_review_entries`、`next_review_details`、`source_review_queue`、`source_review_action_counts`、`suggested_source_kind_counts`、warning薬剤の`t_half_h` source解決率を出力します。さらに warning薬剤を `fully_mapped_warning_drugs` / `partially_mapped_warning_drugs` / `unmapped_warning_drugs` に分けるため、次にどの薬剤をレビューすべきかを確認できます。`source_review_queue` では、薬剤ごとの未解決field、available/used/unused source id、source URL refs、coverage、highest priority、review action をまとめて確認できます。`source_verification` がある未解決entryでは、確認済みsource/query、blocker、次のreview actionも `unresolved_entry_details` / `next_review_details` に出ます。`suggested_source_refs` は source kind/rank 付きで、label, PubMed, journal, DrugBank, PubChem, Wikipedia, secondary の順に確認候補を並べます。`source_id: null` の項目は、値ごとの直接source mappingが未確認であることを明示するために残しています。sourceを確認できた場合だけ `source_id` を `sources[].id` に接続してください。
